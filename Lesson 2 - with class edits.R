...
if(sqrt(49) >= 6.7 & ! "blue" %in% colors | NA == NA | 2^4 == 16){
  print("something")
}
...
starwars$hair_color == "blond" & !is.na(starwars$hair_color)
...
flights %>%
  filter(origin == "JFK" & carrier == "AA" & dep_time > 1200) %>%
  count(dest, sort = TRUE)

# What destination was the most popular?
...
flights_speed <- flights %>%
  ...
  relocate(distance, air_time, speed, 0)
...
flights_holidays <- flights %>%
  ...
  mutate(is_holiday = ifelse(is.na(holiday_name), FALSE, TRUE)) %>%
  relocate(month, day, is_holiday, holiday_name, .before = 0)
...
flights %>%
  group_by(carrier) %>%
  summarize(iqr = IQR(air_time, na.rm = TRUE)) %>%
  arrange(desc(iqr))
...
flights %>%
  group_by(carrier) %>%
  summarize(iqr = IQR(dep_delay, na.rm = TRUE)) %>%
  arrange(desc(iqr))
